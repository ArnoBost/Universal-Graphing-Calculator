...
//a greater scale results into lower unitsPerHashmark:
//  pointsPerUnit=50 => unitsPerHashmark= 1
//  pointsPerUnit= 1 => unitsPerHashmark=50
//  pointsPerUnit= 0 => no axes are drawn
//  pointsPerUnit< 0 => change of direction of x-axis
{
	if (!pointsPerUnit) return; //check if scale factor <> 0

	//draw hashmarks only if axes are visibile within the bounds rect
	...
	int unitsPerHashmark = MIN_PIXELS_PER_HASHMARK * 2 / pointsPerUnit; //calculate unitsPerHashmark depending on given scale parameter
	if (!unitsPerHashmark) unitsPerHashmark = 1; //set 1 as minimum for unitsPerHashmark
